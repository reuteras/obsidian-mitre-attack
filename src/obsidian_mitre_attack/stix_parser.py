            ...
            if ('x_mitre_deprecated' not in tactic or not tactic['x_mitre_deprecated']) and \
                ...
                for technique in techniques_stix:
                    if ('x_mitre_deprecated' not in technique or not technique['x_mitre_deprecated']) and \
                            ('revoked' not in technique or not technique['revoked']):
            ...
            if ('x_mitre_deprecated' not in tech or not tech['x_mitre_deprecated']) and \
                ...
                for relation in procedure_examples_stix:
                    if ('x_mitre_deprecated' not in relation or not relation['x_mitre_deprecated']) and \
                            ('revoked' not in relation or not relation['revoked']):
                ...
                for relation in mitigations_relationships:
                    ...
                    if item not in added and \
                            ('x_mitre_deprecated' not in mitigation or not mitigation['x_mitre_deprecated']) and \
                            ('revoked' not in mitigation or not mitigation['revoked']):
                ...
                if technique_obj.is_subtechnique:
                    parent_techniques = self.src.query([
                        ...
                        Filter('external_references.external_id', '=', technique_obj.main_id)
                    ])
        ...
        for group in groups_stix:
            if ('x_mitre_deprecated' not in group or not group['x_mitre_deprecated']) and \
                ...
                for tech_group_rel in tech_group_relationships:
                    if ('x_mitre_deprecated' not in tech_group_rel or not tech_group_rel['x_mitre_deprecated']) and \
        ...
        for software in software_stix:
            if ('x_mitre_deprecated' not in software or not software['x_mitre_deprecated']) and \
                ...
                for relationship in source_relationships:
                    if relationship['source_ref'].startswith('campaign'):
                        ...
                        for campaign in campaigns_stix:
                            if ('x_mitre_deprecated' not in campaign or not campaign['x_mitre_deprecated']) and \
                ...
                for relationship in target_relationships:
                    if relationship['source_ref'].startswith('intrusion-set') and \
            ...
            for campaign in campaigns_stix:
                if ('x_mitre_deprecated' not in campaign or not campaign['x_mitre_deprecated']) and \
                    ...
                    group_relationships_enterprise = self.enterprise_attack.query([
                        Filter(prop='type', op='=', value='relationship'),
                        Filter(prop='relationship_type', op='=', value='attributed-to'),
                        Filter(prop='source_ref', op='=', value=campaign_obj.internal_id),
                    ])
                    ...
                    software_relationships = software_relationships_enterprise + \
        ...
        for asset in assets_stix:
            if ('x_mitre_deprecated' not in asset or not asset['x_mitre_deprecated']) and \
                ...
                for relationship in asset_relationships:
                    if ('x_mitre_deprecated' not in relationship or not relationship['x_mitre_deprecated']) and \
                            ('revoked' not in relationship or not relationship['revoked']):
                        technique_stix = self.enterprise_attack.query([
                    ...
                    if ('x_mitre_deprecated' not in relationship or not relationship['x_mitre_deprecated']) and \
                        ...
                        for techniques_relationship in techniques_used_stix:
                            ...
                            if technique_stix:
                                ...
                                sys.exit(f"Technique not found: {techniques_relationship['target_ref']} ({data_source_obj.internal_id})")
                        data_component = {
